...
import Price from './style.module.scss';
...
class Cart extends React.Component{
    render(){
        ...
        let productsRows = cartModel.productsDetailed.map((product, i) => {
            return (
                <tr className={Price.price} key={product.id}>
                    <td>{product.title}</td>
                    <td>{product.price}</td>
                    <td>
                        <AppMinMax 
                            ...
                            disable={cartModel.disableElement(product.id)}
                        />
                    </td>
                    <td>{product.price * product.cnt}</td>
                    <td>
                        <button onClick={() => cartModel.remove(product.id)}
                                disabled={cartModel.disableElement(product.id)}
                        >
                            X
                        </button>
                    </td>
